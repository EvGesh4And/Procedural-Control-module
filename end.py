...
class End(QWidget):
    def __init__(self, parent=None):
        """
        Метод определяет блок "Конец", его атрибуты: text - текстовое поле, node_child - для связи с нижеследующим блоком
        """
        super().__init__(parent)
        self.metka_blocka = 'end'
        # Отец
        self.father = None
        # Заглушка для обновления рисовки
        self.son = None
    ...
    def set_name(self, name):
        ...
        self.real_name = name.replace("_end", "") + "_end"
    ...
    def deleted_block(self):
        ...
        self.reply = QMessageBox.question(
            self, "Удаление",
            "Вы действительно хотите удалить блок Конец?",
        ...
        if self.reply == QMessageBox.Yes:
            ...
            # Делаем кнопку добавить блок end снова активной
            self.parent.parent.end_menu.setEnabled(True)
            self.close()
            self.parent.end = None
        else:
            pass
